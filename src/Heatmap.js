...
const Heatmap = ({ inputText }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(
          "https://10.51.3.66:8080/commitment/getDepartmentInfoByYear?date=" +
            inputText +
            "-01-01"
        );
        const data = await response.json();
  ...
  useEffect(() => {
    const fetchAvgWorkload = async () => {
      if (selectedStaff) {
        try {
          const response = await fetch(
            "https://10.51.3.66:8080/commitment/getWorkloadPerStaff?staff-id=${selectedStaff.id}&year=${inputText}"
          );
          const data = await response.json();
          setAvgWorkload(data);
  ...
  useEffect(() => {
    fetch("https://10.51.3.66:8080/courseInstance/getByYear?year=" + inputText)
  ...
  useEffect(() => {
    fetch("https://10.51.3.66:8080/project/getByYear?year=" + inputText)
      .then((response) => response.json())
      .then((data) => {
        setProjectData(data);
      })
